...
const ContextProvider = ({children}) => {
	...
	const addBook = (book) => {
		...
		shallowBooks.push(book);
		setBooks(shallowBooks);
	}
	const addGenre = (genreId, newGenre) => {
		const shallowGenre = genres;
		shallowGenre[genreId].subgenres.push(newGenre);
		setGenre(shallowGenre);
	}
	return(
		<Context.Provider value={{ books, addBook, genres, addGenre}}>
		    {children}
		</Context.Provider>
	    );
}
